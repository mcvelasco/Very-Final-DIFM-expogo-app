...
export default function HomeScreen() {
    ...
    Alert.alert('Sign Out', 'Are you sure you want to sign out?', [
      { text: 'Cancel', style: 'cancel' },
      {
        ...
        onPress: async () => {
          ...
          // _layout.tsx useAuth will detect the session change and redirect automatically
        },
      },
    ]);
  };
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <LinearGradient colors={['#1D4ED8', '#2563EB']} style={styles.header} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
            ...
            ListEmptyComponent={
              <View style={styles.emptyState}>
                ...
                <Text style={styles.emptyBody}>
                  Tap the microphone to record a voice note, or press <Text style={{ fontWeight: '700' }}>+</Text> to type one.
                </Text>
              </View>
            }
          />
      ...
      <TouchableOpacity style={styles.fab} onPress={() => setShowAddModal(true)} activeOpacity={0.85}>
        <LinearGradient colors={['#2563EB', '#3B82F6']} style={styles.fabGradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
      ...
      <Modal visible={showAddModal} transparent animationType="slide" onRequestClose={() => setShowAddModal(false)}>
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.modalOverlay}>
          ...
          <View style={styles.modalCard}>
            ...
            <TextInput
              style={styles.modalInput} placeholder="Write your note here…" placeholderTextColor="#9CA3AF"
              multiline autoFocus value={newNoteText} onChangeText={setNewNoteText} maxLength={2000}
            />
            <Text style={styles.charCount}>{newNoteText.length}/2000</Text>
            <View style={styles.modalActions}>
              <TouchableOpacity style={styles.cancelBtn} onPress={() => { setNewNoteText(''); setShowAddModal(false); }}>
                <Text style={styles.cancelBtnText}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.saveBtn, addingNote && { opacity: 0.6 }]} onPress={handleAddNote} disabled={addingNote}>
                <LinearGradient colors={['#2563EB', '#3B82F6']} style={styles.saveBtnGradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
                  {addingNote ? <ActivityIndicator color="#FFF" size="small" /> : <Text style={styles.saveBtnText}>Save Note</Text>}
                </LinearGradient>
              </TouchableOpacity>
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  fab: {
    position: 'absolute', right: 20, bottom: 28, borderRadius: 28, overflow: 'hidden',
    shadowColor: '#2563EB', shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.4, shadowRadius: 16, elevation: 10,
  },
