...
export default function EditScreen() {
  ...
  const handleSave = async () => {
    if (!content.trim()) {
      Alert.alert('Empty Note', 'A note cannot be empty.');
      return;
    }
    if (content.trim() === initialContent?.trim()) { router.back(); return; }
    try {
      setSaving(true);
  ...
  const handleBack = () => {
    if (content.trim() !== initialContent?.trim()) {
      Alert.alert('Discard Changes?', 'Going back will lose unsaved changes.', [
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <LinearGradient
        colors={['#1D4ED8', '#2563EB']} style={styles.header}
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.flex}>
        <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
          <View style={styles.card}>
            ...
            <TextInput
              style={styles.input} value={content} onChangeText={setContent}
              multiline autoFocus placeholder="Write your note here…"
              placeholderTextColor="#9CA3AF" textAlignVertical="top" maxLength={5000}
            />
...
const styles = StyleSheet.create({
  ...
  input: { fontSize: 15, color: '#111827', lineHeight: 23, minHeight: 220 },
  ...
  cancelBtn: { paddingVertical: 14, alignItems: 'center', borderRadius: 14, borderWidth: 1.5, borderColor: '#E5E7EB' },
